...
index 3df134a..dca2dc0 100644
--- a/panels.py
+++ b/panels.py
@@ -489,6 +489,112 @@ def _cat_legend():
             '</span></div>')
...
@@ -547,16 +653,7 @@ def call_detail(M, day):
...
+            note = _call_note(r)
             body.append(
                 ...
                 '<table class="cdi"><tr>'
@@ -586,6 +683,15 @@ def _az_link(kind, rid, name):
...
@@ -694,18 +800,28 @@ def task_audit_tables(audit, label):
...
**Thinking is disabled on the API call, and that is not an optimisation.**
`claude-sonnet-5` thinks by default. With `max_tokens: 600` it spent the entire
budget thinking on Guillermo Lara's 24-minute call and returned zero text --
`stop_reason: max_tokens`, `thinking_tokens: 600`, no answer -- so the longest
and most informative call of the day silently fell back to producer notes.
Disabling thinking cut output from 1,447 tokens to 314 with identical
objections and verdicts. If a future model rejects the parameter, `_ask` retries
without it at a much larger budget. Do not "simplify" either branch away.

**The producer's name is passed in, never read out of the transcript.** These
calls open with the agency greeting, which names Frank rather than whoever is on
the line -- the first real run had Coral's 24-minute call summarised as "Frank
quoted Guillermo". `_ask` takes the producer's first name from the metrics row
and the prompt forbids taking that identity from the audio.
...
**"None raised" is a claim, not a default.** `objections` comes back empty in
two very different situations: the transcript was read and nobody pushed back,
or nobody read anything (no recording, or no API key). Only the first is a
finding. The renderer prints "None raised" only when `source == "recording"`.
Do not simplify this to `if not objs` — with no API key configured, every row
in the panel is a producer-note fallback, and that shortcut would put a
fabricated "None raised" on all 25 of them.
...
**`PRINT_CSS` in `overflow.py` is not cosmetic.** A Call Detail row that
straddles a PDF page break makes Chromium paint that row's collapsed left border
down the full height of the next page. On the first build, Mary Gunn's yellow
rail ran the whole length of the following page through Mike's section, so every
row under it read as "lost, never quoted". Keeping rows whole fixes it. The rule
is print-only and lives in the standalone PDF document; the email body never
sees it.
